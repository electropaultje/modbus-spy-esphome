...
vector<ModbusData*>* ModbusDataSplitter::handle_function_16(ModbusFrame* request, ModbusFrame* response) {
  ...
  uint16_t expected_bytes_in_response = register_count_requested * 2 + 2;
  if (response->get_data_length() != expected_bytes_in_response) {
    ESP_LOGD(TAG, "Response data length for function 16 does not match expected length: expected %d, got %d",
             expected_bytes_in_response, response->get_data_length());
    return nullptr;
  }

  uint8_t data_bytes = request_data[4];
  if (data_bytes != 2 * register_count_requested)
  {
    ESP_LOGD(TAG, "Number of bytes for function 16 is not %d, but %d", 2 * register_count_requested, data_bytes);
    return nullptr;
  }
/*
...
*/
  ...
  for (uint8_t i = 0; i < register_count_requested; ++i) {
    ModbusData *modbus_data_for_register = new ModbusData;
    modbus_data_for_register->address = start_address + i;
    uint8_t value_high_byte = request_data[i * 2 + 5];
    uint8_t value_low_byte = request_data[i * 2 + 6];
    modbus_data_for_register->value = (value_high_byte << 8) | value_low_byte;
    split_data->push_back(modbus_data_for_register);
  }
/*
